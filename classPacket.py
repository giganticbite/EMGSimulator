...
def convertValueByByte( packet,startIdx,length ):
	...
	icnt=0
	while	icnt<length:
		v += ord(packet[startIdx+icnt])*pow(16,icnt)
		icnt+=1
...
def getSendCommand(sendCommandId, targetSenssorModuleId):
    ...
    if sendCommandId == DEF_SENDCOMMAND_ID_GETSTATUSINFO:
        ...
        return getSendCommand_PreMeasure(targetSenssorModuleId, 0x34,
                                         datetime.datetime.now() + datetime.timedelta(minutes=1))
    elif sendCommandId == DEF_SENDCOMMAND_ID_STARTMEASURE:
        return getSendCommand_StartMeasure(targetSenssorModuleId, 0x34,
                                           datetime.datetime.now() + datetime.timedelta(minutes=1))
...
class ACKPacket(object):
    ...
    def Analyze(self, packet):
        # 2byteはヘッダー分
        self.mBytesLen = ord(packet[2])
        self.mProductId = ord(packet[3])
        self.mTargetSensorModuleId = ord(packet[4])
        self.mResponseCode = ord(packet[5])
        self.mAckStatus = ord(packet[6])
        self.mBcc = ord(packet[7])
...
class CDataPacketCommon(object):
    ...
    def AnalyzeHeader(self, packet):
        self.mBytesLen = ord(packet[2])
        self.mTargetSensorModuleId = ord(packet[3])
        self.mProductId = ord(packet[4])
        self.mResponseCode = ord(packet[5])
        self.mBcc = ord(packet[self.mBytesLen + 2 - 1])
        return 4
...
class CDataPacket_GetStatusInfo(CDataPacketCommon):
    ...
    def Analyze(self, packet):
        ...
        self.mMeasurementHour = ord(packet[7])
        self.mMeasurementMin = ord(packet[8])
        self.mMeasurementSec = ord(packet[9])

        self.mMeasuringfrequency = ord(packet[10]) * 16 + ord(packet[11])
        self.mRadioChannel = ord(packet[12])

        self.mSerialNo = ord(packet[13]) * pow(16, 4) \
                         + ord(packet[14]) * pow(16, 3) \
                         + ord(packet[15]) * pow(16, 2) \
                         + ord(packet[16]) * pow(16, 1) \
                         + ord(packet[17]) * pow(16, 0)

        self.mHardwareVersionNo = ord(packet[18]) * pow(16, 4) \
                                  + ord(packet[19]) * pow(16, 3) \
                                  + ord(packet[20]) * pow(16, 2) \
                                  + ord(packet[21]) * pow(16, 1) \
                                  + ord(packet[22]) * pow(16, 0)

        self.mMemoryAddress = ord(packet[70]) * pow(16, 4) \
                              + ord(packet[71]) * pow(16, 3) \
                              + ord(packet[72]) * pow(16, 2) \
                              + ord(packet[73]) * pow(16, 1)
        self.mMemoryFileCount = ord(packet[74])
        self.mBatteryValue = ord(packet[75])
        self.mTrafficRoute = ord(packet[76])
        self.mCpuTemperature = ord(packet[77])
...
class CDataPacket_StartMeasure(CDataPacketCommon):
    ...
    def Analyze(self, packet):
        ...
        self.mBatteryVoltage = ord(packet[6])
...
class CDataPacket_GetFileInformation(CDataPacketCommon):
    ...
    def Print(self):
        ...
        print("mCalcSensorModuleFixSetting:" + str(self.mCalcSensorModuleFixSetting))
...
def AnalyzePacketThread(packetBufferBytes):
    ...
    # print	"packet len():" + str(packetLength)
    ...
    while icnt < packetLength:
        nowByte = packetBufferBytes[icnt]
        ...
        if checkHeader(prevByte, nowByte) == True:
            ...
            byteLength = ord(packetBufferBytes[icnt + 1])
            # print	"byteLength:",str(byteLength)
            # print	"footer:",ord(packetBufferBytes[ icnt + 1 + byteLength])

            # フッターをチェック
            footerIndex = icnt + 1 + byteLength
            # パケット長をオーバーしていないかをチェック
            if footerIndex < packetLength:
                if checkFooter(packetBufferBytes[icnt + 1 + byteLength]) == True:
                    ...
                    # 応答コードによって、分岐処理
                    responseCode = ord(packetBufferBytes[topIndex + 5])
                    ...
                    if ackPacket != None:
                        # ACKパケットが有効
                        ackPacket.Analyze(packetBufferBytes[topIndex:topIndex + 2 + byteLength])
                        resultDic['ack'] = ackPacket

                    if dataPacket != None:
                        # データパケットが有効
                        dataPacket.Analyze(packetBufferBytes[topIndex:topIndex + 2 + byteLength])
